...
<li class="rounded-md grid gap-4 lg:gap-8 lg:grid-cols-[400px_400px] md:grid-cols-[352px_auto] lg:w-fit">
    <a href="/news/{{ $post['slug'] }}" class="overflow-hidden w-full block rounded-md">
        @if ($post['thumbnail'])
            <img src="{{ $post['thumbnail'] }}" alt="thubmnail of {{ $post['slug'] }}" width="800" height="800"
                class="w-full h-[224px] object-cover hover:scale-105 duration-300" />
        @else
            <div class="w-full h-[224px] bg-gray-900 text-gray-600 flex items-center justify-center">
    ...
    <div class="flex flex-col justify-between gap-3">
        <div>
            <h3 class="{{ 'text-xl lg:text-2xl font-semibold mb-2 ' . ($dark ? 'text-gray-100' : 'text-gray-900') }}">
        ...
        <x-button href="/news/{{ $post['slug'] }}" class="self-end lg:self-start"
